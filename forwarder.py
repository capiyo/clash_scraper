...
class Forwarder:
    ...
    def _format_bson_date(self, ts) -> Dict[str, str]:
        """
        MongoDB Extended JSON date wrapper, e.g. {"$date": "2026-07-19T15:49:20.191Z"}.

        Needed specifically for fields typed as mongodb::bson::DateTime on
        the Rust side (as opposed to chrono::DateTime<Utc>, which accepts
        a bare RFC3339 string fine). serde_json deserializing into
        bson::DateTime -- which is what Axum's Json extractor does, since
        the request body is plain JSON, not real BSON -- only accepts the
        relaxed/canonical Extended JSON date shape, not a bare string.
        Confirmed by CommentaryEntry.created_at (models/game.rs) being
        `#[serde(rename = "createdAt")] pub created_at: BsonDateTime` --
        sending a bare string there was causing every /commentary/bulk
        and /commentary call to 422, even though add_commentary_bulk
        overwrites created_at server-side right after deserializing --
        the bad shape fails before that overwrite line ever runs.
        """
    ...
    # ============================================================
    #
    # NOTE: sub-fixture market CREATION does not happen here anymore.
    # It was originally a POST to /sub_fixtures/sub-fixture/create, which
    # 404'd in production -- that route was never meant to exist.
    # SubFixtureMarket uses #[serde(rename_all = "camelCase")], the same
    # pattern as Game, meaning markets are meant to be written directly
    # into MongoDB the same way fixtures are (see FixtureStore.
    # create_sub_fixture_markets in mongo_store.py), not POSTed to Rust.
    # Settlement below is different -- it's a real financial transaction
    # (paying out pledges), so it correctly stays an HTTP call.
    ...
    def settle_sub_fixture_market(self, match_id: str, market_id: str, winning_team: Optional[str]) -> bool:
        ...
        against the `bets` collection. This hits the route added to
        ...
        ASSUMPTION (UNCONFIRMED, ported from wembly_leagues_scrapers): the
        routes are nested as "/api/sub_fixtures" + "/sub-fixture/settle" in
        main.rs, and self.api_url already ends in "/api" (it does by
        default). That makes the path relative to api_url
        "/sub_fixtures/sub-fixture/settle". Verify against the actual
